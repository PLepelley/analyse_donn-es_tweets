...
class Connexion:
    ...
    def load_infile_table(cls):
        ...
        for index, ligne in df_tweets.iterrows():
            query2 = f"INSERT IGNORE INTO tweets (hashtags, retweetcount, year, month, day, hour, minute, userid) VALUES ('{ligne['hashtags']}', {ligne['retweetcount']}, {ligne['year']}, {ligne['month']}, {ligne['day']}, {ligne['hour']}, {ligne['minute']}, {ligne['user_id']});"
            # print(query2)
            try:
                cls.__cursor.execute(query2)
            except:
    ...
    def procedure(cls, hashtag):
        ...
        cls.__cursor.callproc(query_procedure)

        query_call = f"CALL nb_hashtags_hour('{hashtag}');"
        result = cls.__cursor.callproc(query_call)
        cls.close_connexion()
